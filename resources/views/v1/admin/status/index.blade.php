        ...
        <div class="row">
            <div class="col">
                <div class="card border-primary mb-3" >
                    <p class="text-center"><b>Status</b></p>                    
                    
                    <div class=""></div>
                            <a href="{{ route('status.create') }}" class="bg-emerald-700 hover:bg-emerald-500 text-white font-bold py-2 px-2 rounded-md text-sm">
                                Add New
                            </a>
                        </div>
                        <br/>
                        ...
                        <table class="table-fixed w-full bg-white rounded-lg shadow overflow-hidden">
                                <thead class="text-sm font-medium text-left text-gray-700">
                                    <tr>
                                        ...
                                        <th class="px-4 py-2 w-12">Status</th>
                                        <th class="px-4 py-2 w-20">Status Name</th>
                                ...
                                <tbody>
                                    @foreach ($status as $status )
                                        <tr class="hover:bg-gray-100 border-b border-gray-200 ">
                                            ...
                                            <td class="px-4 py-2 ">{{ $status->status }}</td>
                                            <td class="px-4 py-2 w-20">
                                                <form method="POST" action="{{ route('status.destroy', $status->id) }}">
                                                    <button class="rounded-md bg-blue hover:bg-blue-100 text-white font-bold  px-2 mr-2 py-1 text-sm"><a href="{{ route('status.edit', $status->id) }}">  Edit </a> </button>
                                                    @csrf
                                                    @method('DELETE')
