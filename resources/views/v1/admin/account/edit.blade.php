    ...
    <div class="row">
        <div class="col">
            <div class="card border-primary mb-3 py-3 px-3" style="width: 50rem">
                ...
                <form class="w-full max-w-lg md:items-center" action="{{ route('admins.update', $admin->id) }}" method="post" enctype="multipart/form-data">
                  @csrf
                  @method('PUT')
                  <label class="ml-5">Username</label><br/>
                  <input type="text" name="brandcode" id="brandcode" value="{{ $admin->username }}" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-400 rounded py-3 px-4 mb-3 ml-5 leading-tight focus:outline-none focus:bg-white disabled"><br/>
